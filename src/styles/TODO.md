...
[x] allow me to skip through the four initial forced problems, with a (saved) "skip tutorial problems" checkbox in settings (Done: Added "Skip Tutorial Problems" switch in Settings modal backed by localStorage 'algebra_skip_tutorial'. GameController starts directly on generated multi-family equations or skips tutorial levels immediately when checked).

[x] On mobile, it uses a portrait camera most of the time, and has the answers on the right. This works really well. Except, during some states it goes back to having a 4:3 camera box, which is awkward, these states where it goe sback to 4:3 are {waiting for an answer, story mode before the "which equation matches" comes out, equation is presenting up the top, before the user has selected anything}. I think it is the states where there is no selection menu on the right. I just want the camera to stay in the same spot (see nroe below too) (Done: Updated responsive.css so .camera-box unconditionally maintains its left-docked portrait aspect-ratio across all mobile portrait states, eliminating the jump back to centered 4:3 when docked side panels are hidden).

[x] In story mode, each line of the story comes out one at a time. This is good, but it charges the size of the story block, and it moves the camera box around. I'd rather the camera box just stayed where it was vertically, and we know that the story will have a maximum of 3 lines plus the question, so we put the camera box into the place for that length of story, so it doesn't jump about. Make sense? (Done: Updated storyView.ts to render all beats from the start with unrevealed ones hidden via .sentence-pending visibility: hidden, and added min-height in CSS for 3 lines plus question so the story block height stays completely constant and the camera box never jumps vertically).

[x] when it comes to "sweeping the identity" (eliminating things like -7 + 7) my finger laser does this. However often it doesn't happen first time. This is because we have some logic to stop it automatically hapening if your finger is pointing towards it. However this logic is too aggressive. Currently, if I'm pointing up to the equation bar (but not at the "-7 + 7" box), and then a move my finger over the "-7 + 7" box, it should sweep that box away! But it doesn't, because it thinks we were pointing at it. I only want that "point away" protection to be for the the "-7 +7" box itself, not the whole equation bar. (Done: Fixed modeBForgeHandler.ts so cleanupMustExitTarget is only set upon entering awaiting_cleanup if the laser is ALREADY intersecting the mode-b-cleanup-target box itself. Pointing anywhere else on the rail/equals allows sweeping the identity box away immediately on the very first pass. Also refined the cleanup target hitbox padding from 32px to 16px).

[x] Can we please make it remember that I have given it camera access before on mobile browser? I'm playing on Safari / iPhone and I have to ask every time. (Done: Persisted camera state in localStorage 'algebra_camera_enabled'. On launch, the app auto-attempts to start the camera if previously enabled without showing the banner prompt. Added a "Remember Camera Access" toggle in settings and clear instructions for iOS Safari users on how to set Website Settings → Camera → Allow).
