...
[ ] make the layout fully responsive to different window sizes. I need everything to stay playable in any window size down to 768 height, but I can drag it out to any size on a 4k display, too. Right now only the camera resizes, but everything should resize. A key aspect of the resizing is the selectable elements on the left and right of the camera should span the full width of the camera. i.e. when "forging the opposite", the "+-x/" selectable buttons should span the height of the camera area, making them easy to hit; the header of "choose the opposite" should actually be vertically above the top of the camera pane, becaus the hitable buttons should align.
